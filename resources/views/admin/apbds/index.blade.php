...
@section('button')
    <div id="kt_toolbar_container" class="container-fluid d-flex">
        ...
        <!--end::Page title-->
        <div class="me-5 d-flex align-items-center gap-2">
            <label class="form-label" style="flex: none">Filter Tahun :</label>
            <select name="filter_tahun" class="form-select" data-control="select2" id="filter_tahun"
                data-placeholder="Silahkan pilih tahun">
                <option></option> {{-- placeholder --}}
                @for ($tahun = $tahunSekarang; $tahun >= 2015; $tahun--)
                    <option value="{{ $tahun }}"
                        {{ request('filter_tahun', $tahunSekarang) == $tahun ? 'selected' : '' }}>
                        {{ $tahun }}
                    </option>
                @endfor
            </select>
                        ...
                        <div class="container">
                            <div class="py-5 table-responsive">
                                <table id="kt_table_data"
                                    ...
                                    <tfoot id="tfoot" class="bg-info rounded">
                                        <tr class="fw-bolder fs-6 text-white">
                                            <td style="text-align: center !important;" colspan="5">Surplus / Defisit</td>
            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <textarea name="uraian" class="form-control" rows="2"></textarea>
                        <small class="text-danger uraian_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'APBDS');
        })
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/infografis/apbds-add', 'Tambah',
                    'APBDS',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/infografis/apbds-update/' + uuid,
                    'Update',
                    'APBDS', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/infografis/apbds-show/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'APBDS', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/infografis/apbds-delete/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async (year) => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/infografis/apbds-get/' + year,
                columns: [{
                        data: null,
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="APBDS" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                footerCallback: function(row, data, start, end, display) {
                    ...
                    data.forEach(function(item) {
                        if (item.jenis === 'belanja') {
                            belanja += parseFloat(item.jumlah);
                        } else if (item.jenis === 'pendapatan') {
                            pendapatan += parseFloat(item.jumlah);
                        }
                    });
                    // Calculate surplus/deficit
                    let pemasukan = pendapatan - belanja;

                    // Update the total row in the footer
                    $('#total-saldo').html('Rp ' + numeral(pemasukan).format('0,0'));
                },
            });
        };

        $(function() {
            initDatatable('{{ $tahunSekarang }}');
        });

        // $('#export-excel').click(function(e) {
        //     e.preventDefault();
        //     window.open(`/admin/infografis/apbds-export`, "_blank");
        // });
    </script>
@endsection
