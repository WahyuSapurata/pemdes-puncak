...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            window.location.href = '/admin/surat/surat-keluar-tambah';
        })

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            window.location.href = '/admin/surat/surat-keluar-edit/' + $(this).attr('data-uuid');
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/surat/surat-keluar-delete/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                processing: true,
                ajax: '/admin/surat/surat-keluar-get',
                columns: [{
                    data: null,
                    ...
                    render: function(data, type, row, meta) {
                        let result;
                        result =
                            `<a href="{{ asset('/public/suratkeluar/${data}') }}" target="_blank" class="btn btn-outline btn-outline-dashed btn-outline-danger btn-active-light-danger p-2 py-1">
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            <div class="d-flex gap-2" style="flex-flow: wrap;">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Surat Keluar" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
