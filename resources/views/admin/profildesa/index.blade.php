...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            window.location.href = '/admin/profil-desa-tambah';
        })

        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            window.location.href = '/admin/profil-desa-edit/' + $(this).attr('data-uuid');
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/profil-desa-delete/' + $(this).attr('data-uuid');
                ...
                columns: [{
                            ...
                            const listItems = data.map(function(item, index) {
                                return `
                                        <li class="list-group-item">
                                            <span class="badge bg-primary">${index + 1}</span> ${item.misi}
                                        </li>
                                    `;
                            }).join('');
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        result =
                            `
                                <!--begin::Overlay-->
                                <a class="d-block overlay fancybox" data-fancybox="lightbox-group" href="{{ asset('/public/profildesa/${data}') }}">
                                    <!--begin::Image-->
                                    <div class="overlay-wrapper bgi-no-repeat bgi-position-center bgi-size-cover card-rounded min-h-175px"
                                        style="background-image:url('/public/profildesa/${data}')">
                                    </div>
                                    <!--end::Image-->
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            <div class="d-flex gap-2" style="flex-flow: wrap;">
                                ...
                                <a href="javascript:;" type="button" data-uuid="${data}" data-label="Profil Desa" class="btn btn-danger button-delete btn-icon btn-sm">
                                    <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
